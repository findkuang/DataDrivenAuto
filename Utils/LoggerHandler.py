#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2020/9/22 15:46
# @Author  : kuangxiaojiang
# @File    : LoggerHandler.py
# @Function: 日志处理
import logging
from Utils.GetConfigInfo import LOG_INFO, LOG_FILE_NAME
...
def logger(log_name='接口测试'):
    return LoggerHandler(
        log_name=log_name,
        logger_level=LOG_INFO['LOG_LEVEL'],
        file_name=LOG_FILE_NAME,
        stream_level=LOG_INFO['LOG_STREAM_LEVEL'],
        file_level=LOG_INFO['LOG_FILE_LEVEL']
    ).get_logger


if __name__ == '__main__':
    logger().debug('aaaa', 'bbb')
    logger().info('aaaa')
    logger().warning('aaaa')